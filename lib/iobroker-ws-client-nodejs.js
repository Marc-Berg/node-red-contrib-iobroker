/*!
 * ioBroker WebSocket Client - Fixed to prevent multiple socket creation
 * CRITICAL FIX: Only creates ONE socket per connection attempt
 */
...
class FixedSingleSocketClient {
    constructor() {
        ...
        // CRITICAL: Connection state tracking
        this.isConnecting = false;
        this.connectionPromise = null;

        this.log = {
            debug: (text) => DEBUG && console.log(`[${new Date().toISOString()}] [FixedClient] ${text}`),
            warn: (text) => console.warn(`[${new Date().toISOString()}] [FixedClient] ${text}`),
            error: (text) => console.error(`[${new Date().toISOString()}] [FixedClient] ${text}`),
        };
    ...
    /**
     * FIXED Connect - Prevents multiple socket creation
     */
    async connect(url, options) {
        // CRITICAL: If already connecting, return the existing promise
        if (this.isConnecting && this.connectionPromise) {
            this.log.debug('ALREADY CONNECTING - returning existing promise');
            return this.connectionPromise;
        }
        
        // CRITICAL: If already connected, return immediately
        if (this.connected && this.socket) {
            this.log.debug('ALREADY CONNECTED - returning existing connection');
            return this;
        }

        this.log.debug('Starting SINGLE connection attempt');

        // Mark as connecting and create connection promise
    ...
    async _performConnection(url, options) {
        try {
            ...
            // Get OAuth2 Token only if authentication is enabled
            if (this.useAuthentication) {
            ...
            // CRITICAL: Only create ONE WebSocket
            const wsUrl = this.constructWebSocketUrl(this.url);
            const headers = this.createHeaders();
            
            this.log.debug(`Creating SINGLE WebSocket to: ${wsUrl}`);
            
            // CRITICAL: Close any existing socket first
            if (this.socket) {
            ...
            this.log.debug('SINGLE WebSocket instance created');
...
function connect(url, options) {
    const socketClient = new FixedSingleSocketClient();
...
module.exports = {
    connect,
    SocketClient: FixedSingleSocketClient
};