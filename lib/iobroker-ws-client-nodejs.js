...
class SocketClient {
    constructor() {
        ...
        this.log = {
            debug: (text) => {
                ...
                if (text.includes('destroyed') || text.includes('OAuth')) {
                    console.log(`[${new Date().toISOString()}] [WebSocketClient:${this.clientId}] ${text}`);
                }
            },
            warn: (text) => console.warn(`[${new Date().toISOString()}] [WebSocketClient:${this.clientId}] ${text}`),
            error: (text) => console.error(`[${new Date().toISOString()}] [WebSocketClient:${this.clientId}] Error: ${text}`),
        };
    ...
    createHeaders() {
        ...
        if (this.useAuthentication && this.accessToken && this.accessToken.trim()) {
            ...
            // Empty Cookie header to prevent undefined.split() crash
