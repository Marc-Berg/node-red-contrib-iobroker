/*!
 * ioBroker WebSocket Client with Proactive Token Refresh and SSL Support
 */
...
class SocketClient {
    constructor() {
        ...
        this.useSSL = false; // New SSL configuration
        ...
        this.log = {
            debug: (text) => {
                if (text.includes('destroyed') || text.includes('OAuth') || text.includes('token') || text.includes('SSL')) {
                    const now = new Date();
    ...
    determineSSLUsage(host, port, explicitSSL) {
        // Priority: explicit SSL setting > port-based detection > default false
                    ...
                    try {
                        if (res.statusCode === 200) {
                            ...
                            if (tokenData.access_token) {
                                this.log.debug('OAuth token received successfully via ' + (this.useSSL ? 'HTTPS' : 'HTTP'));
    ...
    async _performConnection(url, options) {
        try {
            ...
            // SSL specific WebSocket options
            if (this.useSSL) {
                wsOptions.rejectUnauthorized = false; // Accept self-signed certificates
                wsOptions.ca = undefined; // Don't use system CA for self-signed
            }
