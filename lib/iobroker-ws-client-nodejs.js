...
class SocketClient {
    constructor() {
        ...
        this.maxRetryAttempts = 50; // After 50 attempts, switch to production mode
        this.baseRetryDelay = 2000; // Start with 2 seconds
        this.maxRetryDelay = 30000; // Max 30 seconds between retries
        this.productionRetryDelay = 60000; // 1 minute between retries in production mode
        this.retryTimer = null;
        this.permanentFailure = false;
        this.isInitialConnection = true;
        this.connectionRecoveryEnabled = true;
        this.productionMode = false; // Switch to slower, infinite retries after maxRetryAttempts
        ...
        this.log = {
            debug: (text) => {
                if (text.includes('destroyed') || text.includes('OAuth') || text.includes('token') || 
                    text.includes('retry') || text.includes('recovery') || text.includes('production')) {
                    console.log(`[debug] [WebSocketClient:${this.clientId}] ${text}`);
                }
            },
            info: (text) => console.log(`[info] [WebSocketClient:${this.clientId}] ${text}`),
    ...
    // Enable/disable connection recovery
    ...
    calculateRetryDelay() {
        // In production mode (after initial attempts), use fixed longer delay
        if (this.productionMode) {
            const jitter = Math.random() * 5000; // Add up to 5 seconds jitter
            return this.productionRetryDelay + jitter;
        }
        
        // Normal exponential backoff for initial attempts
    ...
    // Schedule connection retry with production mode support
    scheduleRetry() {
        ...
        // Switch to production mode after initial attempts
        if (this.retryAttempts >= this.maxRetryAttempts && !this.productionMode) {
            this.productionMode = true;
            this.log.info(`Switching to production retry mode - will continue retrying every ${this.productionRetryDelay/1000}s indefinitely`);
            this.log.debug(`Production mode enabled after ${this.maxRetryAttempts} initial connection attempts`);
        }
    ...
    // Start health check for established connections
    startHealthCheck() {
    ...
    async connect(url, options) {
        ...
        // If connected and not retrying, return immediately
    ...
    _reconnect() {
        ...
        // Always attempt reconnection - production mode enables infinite retries
