...
class StateHandler {
    ...
    async unsubscribe(stateIdOrPattern) {
        // Clean local bookkeeping immediately, regardless of backend outcome
        ...
        return new Promise((resolve) => {
            try {
                const timeout = setTimeout(() => {
                    // Timeout: resolve anyway to avoid blocking shutdowns
                    this.client.log.debug(`Unsubscribe timeout locally for ${stateIdOrPattern}`);
                    resolve();
                }, 2000);

                this.client.emit('unsubscribe', stateIdOrPattern, () => {
                    clearTimeout(timeout);
                    resolve();
                });
            } catch (e) {
                // If emit fails (client destroyed), resolve
                resolve();
            }
        });
    }
