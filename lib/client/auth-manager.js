...
const TOKEN_EXPIRY_TIME = 60 * 60 * 1000; // 60 minutes
...
class AuthManager {
    ...
    async authenticate(options) {
        ...
        try {
            ...
            // Don't wrap network errors as authentication errors
            throw authError;
        }
    }
    ...
    async getOAuthToken(host, port, username, password) {
        ...
        this.client.log.debug(`Getting OAuth token for user: ${cleanUsername} via ${this.client.useSSL ? 'HTTPS' : 'HTTP'}`);
        ...
        return new Promise((resolve, reject) => {
                    ...
                    try {
                        if (res.statusCode === 200) {
                            ...
                            if (tokenData.access_token) {
                                this.client.log.debug('OAuth token received successfully');
            ...
            req.on('error', (err) => {
                if (!this.client.destroyed) {
                    // Don't wrap network errors as authentication errors
                    // Let them bubble up as-is so they can be properly classified
                    reject(err);
                }
            });
        ...
        this.tokenRefreshTimer = setTimeout(async () => {
            if (!this.client.destroyed && this.client.useAuthentication && this.client.connected) {
                try {
                    ...
                    this.client.log.error(`Scheduled token refresh failed: ${error.message}`);
                }
            }
        }, TOKEN_REFRESH_INTERVAL);

        this.client.log.debug(`Token refresh scheduled in ${TOKEN_REFRESH_INTERVAL / 60000} minutes`);
    }
    ...
    async refreshTokenProactively() {
        ...
        try {
            ...
            this.client.log.error(`Token refresh failed: ${error.message}`);
            throw error;
        } finally {
            this.isRefreshingToken = false;
            this.refreshPromise = null;
    ...
    async performTokenRefresh() {
        try {
            this.client.log.debug('Starting proactive token refresh with session renewal');
            ...
            this.client.log.debug(`Token and session refreshed successfully (${oldSessionId} -> ${this.client.sessionID})`);
    ...
    async rebuildConnection() {
        ...
        try {
            this.client.log.debug('Rebuilding WebSocket connection with new session and token');
            ...
            this.client.log.debug('WebSocket connection rebuilt successfully');
            
        } catch (error) {
            this.client.log.error(`Failed to rebuild connection: ${error.message}`);
            this.client.close();
            throw error;
        }
    }
        ...
        if (this.client.handlers['tokenRefresh']) {
            this.client.handlers['tokenRefresh'].forEach(cb => {
                try {
                    ...
                    this.client.log.error('Error in token refresh callback:', callbackError);
                }
            });
        }
    }
    ...
    handleReauthenticate() {
        if (this.client.useAuthentication && !this.client.destroyed) {
            ...
            this.client.log.debug('Reauthenticate request received - triggering session renewal');
            this.refreshTokenProactively().catch(error => {
                this.client.log.error(`Reauthenticate session renewal failed: ${error.message}`);
