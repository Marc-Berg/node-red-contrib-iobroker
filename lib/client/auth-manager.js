    ...
    async triggerResubscriptionAndWait() {
        try {
            ...
            if (this.client.socket && this.client.connected) {
                ...
                this.log.debug(`Waiting for server-side ready signal for session ${targetSessionId}`);

                const waitForReady = new Promise((resolve) => {
                    const startTime = Date.now();
                    const pollIntervalMs = 25;

                    const checkReadySignal = () => {
                        if (this.client.destroyed) {
                            this.log.debug('Aborting ready wait - client destroyed');
                            resolve(subscriptionCount);
                            return;
                        }

                        if (this.client.lastReadySessionID === targetSessionId) {
                            const waitedMs = Date.now() - startTime;
                            this.log.debug(`Server-side ready received for session ${targetSessionId} after ${waitedMs}ms`);
                            resolve(subscriptionCount);
                            return;
                        }
                        ...
                        if (elapsedMs >= waitTimeoutMs) {
                            this.log.warn(`Ready wait timeout for session ${targetSessionId} after ${elapsedMs}ms`);
                            resolve(subscriptionCount);
                            return;
                        }

                        setTimeout(checkReadySignal, pollIntervalMs);
                    };
