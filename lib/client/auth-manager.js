...
class AuthManager {
    ...
    async performTokenRefresh() {
        try {
            this.log.info('Starting proactive token refresh with session renewal');
            ...
            this.log.info(`Token and session refreshed successfully (${oldSessionId} -> ${this.client.sessionID})`);
    ...
    async rebuildConnection() {
        ...
        try {
            this.log.info('Rebuilding WebSocket connection with new session and token');
            ...
            this.log.info('WebSocket connection rebuilt successfully');
    ...
    handleReauthenticate() {
        if (this.client.useAuthentication && !this.client.destroyed) {
            ...
            this.log.info('Reauthenticate request received - triggering session renewal');
