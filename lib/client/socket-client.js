/*!
 * Core WebSocket Client for ioBroker
 */

const AuthManager = require('./auth-manager');
const ConnectionRecovery = require('./connection-recovery');
...
class SocketClient {
    ...
    async getState(stateId, useCache = false) {
        ...
        return await this.stateHandler.getState(stateId, useCache);
    }
    ...
    async requestInitialValue(stateId) {
        ...
        return await this.stateHandler.requestInitialValue(stateId);
    }
    ...
    getConnectionStats() {
        const stats = {
            ...
            cachedStates: this.stateHandler.stateCache.size,
        ...
        // Add recovery stats
        Object.assign(stats, this.connectionRecovery.getStats());

        return stats;
    }

    setConnectionRecovery(enabled) {
        this.connectionRecovery.setEnabled(enabled);
    }
    ...
    async connect(url, options) {
        ...
        if (this.connected && this.socket && this.connectionRecovery.retryAttempts === 0) {
            return this;
        }
        ...
        try {
            ...
            if (this.connectionRecovery.isRetryableError(error)) {
                this.log.debug(`Connection failed, will retry: ${this.lastError}`);
                this.connectionRecovery.scheduleRetry();
            } else {
                this.log.error(`Connection failed permanently: ${this.lastError}`);
                this.connectionRecovery.permanentFailure = true;
            }
    ...
    _handleConnection() {
        ...
        this.connectionRecovery.resetRetryState();
    ...
    startHealthCheck() {
        if (this.healthCheckInterval || !this.connectionRecovery.connectionRecoveryEnabled) {
            return;
        }
    ...
    _reconnect() {
        if (this.destroyed || !this.connectionRecovery.connectionRecoveryEnabled) {
            return;
        }
        ...
        if (!this.connectionRecovery.permanentFailure) {
            this.connectionRecovery.scheduleRetry();
        }
    }
