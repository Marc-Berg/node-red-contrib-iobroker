...
class SocketClient {
    constructor() {
        ...
        this.sessionID = 0;
        this.lastReadySessionID = null;
        this.lastResubscriptionSessionID = null;
    ...
    constructWebSocketUrl(url) {
        ...
        const urlForLog = wsUrl.replace(/token=[^&]+/, 'token=***');
        this.log.debug(`WebSocket URL constructed: ${urlForLog}`);
        return wsUrl;
    }
    ...
    clearAllTimers() {
        ...
        if (this.tokenReauthTimer) {
            clearTimeout(this.tokenReauthTimer);
            this.tokenReauthTimer = null;
        }
    }
    ...
    createHeaders() {
        ...
        // Only append port if explicitly set (not default 80/443)
        const port = urlObj.port || (this.useSSL ? '443' : '80');
        const hostHeader = (port === '80' && !this.useSSL) || (port === '443' && this.useSSL)
            ? urlObj.hostname
            : `${urlObj.hostname}:${port}`;
        const originHeader = (port === '80' && !this.useSSL) || (port === '443' && this.useSSL)
            ? `${this.useSSL ? 'https' : 'http'}://${urlObj.hostname}`
            : `${this.useSSL ? 'https' : 'http'}://${urlObj.hostname}:${port}`;
    ...
    setupWebSocketHandlers() {
        ...
        this.socket.onopen = () => {
            ...
            // Log negotiated WebSocket extensions (e.g., permessage-deflate)
            try {
                const exts = this.socket ? this.socket.extensions : null;
                if (!exts) {
                    this.log.info('Negotiated WS extensions: none');
                } else if (typeof exts === 'string') {
                    const list = exts.trim();
                    this.log.info(`Negotiated WS extensions: ${list || 'none'}`);
                    if (list.toLowerCase().includes('permessage-deflate')) {
                        this.log.info('permessage-deflate enabled');
                    }
                } else if (typeof exts === 'object') {
                    const names = Object.keys(exts);
                    this.log.info(`Negotiated WS extensions: ${names.length ? names.join(', ') : 'none'}`);
                    if (names.includes('permessage-deflate')) {
                        this.log.info('permessage-deflate enabled');
                    }
                } else {
                    this.log.info('Negotiated WS extensions: unknown format');
                }
            } catch (e) {
                this.log.debug(`WS extensions check failed: ${e.message}`);
            }
        ...
        try {
            if (type === MESSAGE_TYPES.CALLBACK) {
                ...
                if (name === '___ready___') {
                    ...
                    const rawExp = args && args[0] && (args[0].expiresAt || args[0].exp);
                    const parsedExp = Number(rawExp);
                    const exp = Number.isFinite(parsedExp)
                        ? (parsedExp < 1e12 ? parsedExp * 1000 : parsedExp)
                        : NaN;
                    ...
                    if (Number.isFinite(exp)) {
                        ...
                        this.authManager.cancelFallbackTimer();
                        const delay = exp - Date.now() - 10000;
                        const effectiveDelay = Math.min(Math.max(delay, 0), 2147483647);
                        this.log.debug(`tokenInfo received: exp=${new Date(exp).toISOString()}, reauthAt=${new Date(exp - 10000).toISOString()}, delayMs=${effectiveDelay}`);
                        this.tokenReauthTimer = setTimeout(() => {
                            this.log.debug(`tokenInfo reauth timer fired for session ${this.sessionID}`);
                            this.tokenReauthTimer = null;
                            this.authManager.handleReauthenticate();
                        }, effectiveDelay);
                    } else {
                        this.log.debug('tokenInfo received without valid exp, reauth timer not set');
                    }
    ...
    _handleConnection() {
        ...
        this.connected = true;
        this.lastReadySessionID = this.sessionID;

        this.authManager.startFallbackGrace();
    ...
    removeAllEventListeners() {
        if (this.socket) {
            try {
                ...
                if (this.socket.readyState === WebSocketClass.OPEN ||
                    this.socket.readyState === WebSocketClass.CONNECTING) {
