...
class SocketClient {
        ...
        try {
            if (type === MESSAGE_TYPES.CALLBACK) {
                ...
                if (name === '___ready___') {
                    ...
                    // Handle object changes - NEW functionality
                    if (args && args.length >= 2) {
                        ...
                        // Emit object change event for listeners
                        ...
                        if (args && args.length >= 1) {
                            ...
                            this.log.debug(`Log message received: ${logData.severity || 'unknown'} - ${logData.message || 'no message'}`);
    ...
    _handleConnection() {
        ...
        // Clear connecting timer when ready signal is received
        if (this.connectingTimer) {
            clearTimeout(this.connectingTimer);
            this.connectingTimer = null;
