/*!
 * WebSocket Connection Management Service for ioBroker Integration  
 * Handles WebSocket lifecycle, reconnection strategies, and parallel connection management
 */
...
class ConnectionService {
    ...
    async handleParallelRefresh(config, newToken, oldToken, overlapTime) {
        ...
        try {
            ...
            parallelSocket.on('message', (data) => {
                ...
                // Don't forward messages from parallel connection to avoid duplicates
                // Only handle authentication and readiness signals
                this.logger.trace(`[${serverId}] Message received on parallel connection but not forwarded: ${messageStr.substring(0, 100)}...`);
            });
    ...
    switchToParallelConnection(serverId) {
        ...
        eventBus.emit('connection:token_refreshed', { serverId, ws: state.socket, isReconnect: true });
    }
