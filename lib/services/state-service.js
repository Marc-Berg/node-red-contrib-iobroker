/*!
 * State Management Service for ioBroker Integration
 * Handles WebSocket message processing, state subscriptions, and data coordination
 */
...
class StateService {
    constructor() {
        ...
        // Deduplication for state changes (prevents duplicate events from overlapping subscriptions)
        this.recentStateChanges = new Map(); // stateId -> { timestamp, timeout }
    ...
    cleanupEventListeners() {
        // Remove any existing StateService listeners to prevent duplicates
        eventBus.removeAllListeners('websocket:message');
        eventBus.removeAllListeners('state:subscribe_request');
        eventBus.removeAllListeners('state:unsubscribe_request');
        eventBus.removeAllListeners('state:get_request');
        eventBus.removeAllListeners('state:set_request');
        eventBus.removeAllListeners('states:get_request');
        eventBus.removeAllListeners('object:get_request');
        eventBus.removeAllListeners('enums:get_request');
        eventBus.removeAllListeners('multiple_data:get_request');
        eventBus.removeAllListeners('object:set_request');
        eventBus.removeAllListeners('object:subscribe_request');
        eventBus.removeAllListeners('object:unsubscribe_request');
        eventBus.removeAllListeners('log:subscribe_request');
        eventBus.removeAllListeners('log:unsubscribe_request');
        eventBus.removeAllListeners('auth:success');
        eventBus.removeAllListeners('connection:token_refreshed');
    }
    ...
    setupEventListeners() {
        ...
        eventBus.on('state:unsubscribe_request', ({ serverId, stateId, nodeId }) => this.removeSubscription(serverId, stateId, nodeId));
        ...
        eventBus.on('states:get_request', ({ serverId, requestId }) => this.getStates(serverId, requestId));
        ...
        eventBus.on('auth:success', ({ serverId, isReconnect }) => this.handleServerReady(serverId, isReconnect));
        eventBus.on('connection:token_refreshed', ({ serverId }) => this.handleConnectionSwitched(serverId));
        
        this.logger.debug('Event listeners registered');
    }

    cleanup() {
        this.logger.info('StateService cleanup started');
        
        // Remove all event listeners
        this.cleanupEventListeners();
        
        // Clear timeouts for state change deduplication
        this.recentStateChanges.forEach(({ timeout }) => {
        ...
        this.activeLogSubscriptions.clear();
        this.objectSubscriptions.clear();
        this.activeObjectSubscriptions.clear();
        this.multipleDataCoordinator.clear();
        this.recentStateChanges.clear();
        
        this.logger.info('StateService cleanup completed');
    }
    ...
    addSubscription(serverId, stateId) {
        ...
        if (this.activeSubscriptions.get(serverId).has(stateId)) {
            this.logger.debug(`Subscription for "${stateId}" on server ${serverId} already exists, skipping duplicate subscription.`);
            // For existing subscriptions, don't emit confirmation to prevent duplicate events
            // The subscription is already active and the node should maintain its current state
            return;
        }
    ...
    removeSubscription(serverId, stateId, nodeId) {
        this.logger.info(`Removing subscription for "${stateId}" on server ${serverId} for node ${nodeId}`);
        
        // Remove from active subscriptions
        if (this.activeSubscriptions.has(serverId)) {
            this.activeSubscriptions.get(serverId).delete(stateId);
            
            // If no more active subscriptions for this server, clean up
            if (this.activeSubscriptions.get(serverId).size === 0) {
                this.activeSubscriptions.delete(serverId);
            }
        }

        // Check if any servers still need this state subscription
        if (this.serverSubscriptions.has(stateId)) {
            this.serverSubscriptions.get(stateId).delete(serverId);
            
            // If no servers need this subscription anymore, unsubscribe from server
            if (this.serverSubscriptions.get(stateId).size === 0) {
                this.serverSubscriptions.delete(stateId);
                this.logger.info(`Last subscription for "${stateId}" removed. Sending unsubscribe command to server ${serverId}.`);
                this.sendUnsubscribeCommand(serverId, stateId);
            }
        }
    }

    sendUnsubscribeCommand(serverId, stateId) {
        const msgId = this.messageId++;
        this.pendingSubscriptions.set(msgId, { serverId, stateId, type: 'unsubscribe', timestamp: Date.now() });
        eventBus.emit('websocket:send', { serverId, payload: [3, msgId, "unsubscribe", [stateId]] });
    }
    ...
    getStates(serverId, requestId) {
        const msgId = this.messageId++;
        this.logger.info(`Requesting all states from server ${serverId}`);
        ...
        eventBus.emit('websocket:send', { serverId, payload: [3, msgId, "getStates", []] });
    }
    ...
    subscribeToLogs(serverId, logLevel = 'info', nodeId) {
        ...
        if (existingSubscription) {
            ...
            // However, still emit confirmation event for this node since it's expecting it
            this.logger.info(`Emitting log:subscription_confirmed for existing subscription node ${nodeId}`);
    ...
    handleServerReady(serverId, isReconnect = false) {
        ...
        if (subscriptions && subscriptions.size > 0) {
            this.logger.info(`Re-subscribing to ${subscriptions.size} states after ${isReconnect ? 'reconnection' : 'connection'} for server ${serverId}`);
    ...
    handleConnectionSwitched(serverId) {
        this.logger.info(`Connection switched for server ${serverId}. Re-subscribing to maintain subscriptions.`);
        // Emit auth:success event to trigger proper reconnect handling
        eventBus.emit('auth:success', { serverId, isReconnect: true });
    }
    ...
    handleWebSocketMessage(serverId, data) {
        ...
        if (Array.isArray(message) && message[2] === '___ready___') {
            ...
            // Check if this is a reconnect by seeing if we already have active subscriptions
            const isReconnect = this.activeSubscriptions.has(serverId) && this.activeSubscriptions.get(serverId).size > 0;
            eventBus.emit('auth:success', { serverId, isReconnect });
            return;
        }
        ...
        if (Array.isArray(message) && message[0] === 3 && message.length >= 4) {
            ...
            if (pending && command === 'unsubscribe' && pending.type === 'unsubscribe') {
                this.pendingSubscriptions.delete(messageId);
                // result[0] is null for success, string for error
                if (result && result[0] !== null) {
                    this.logger.error(`Unsubscribe failed for ${pending.stateId} on ${serverId}: ${result[0]}`);
                } else {
                    this.logger.info(`Unsubscribe successful for ${pending.stateId} on ${serverId}`);
                }
                return;
            }
            ...
            // Handle getStates responses (for all states)
            if (pending && command === 'getStates' && pending.type === 'getStates') {
                this.pendingSubscriptions.delete(messageId);
                // result[0] is null for success, string for error
                if (result && result[0] !== null) {
                    this.logger.error(`GetStates failed on ${serverId}: ${result[0]}`);
                    eventBus.emit(`states:get_result:${pending.requestId}`, { 
                        error: result[0]
                    });
                } else {
                    this.logger.info(`GetStates successful on ${serverId}`);
                    // result[1] contains the states object
                    const states = result && result[1];
                    eventBus.emit(`states:get_result:${pending.requestId}`, { 
                        result: states
                    });
                }
        ...
        if (Array.isArray(message) && message[2] === 'stateChange') {
            ...
            if (state) {
                this.logger.debug(`State change for ${stateId}: ${state.val}. Checking for duplicates.`);
                
                // Check for recent duplicate events (within 50ms window)
                const now = Date.now();
                const recentChange = this.recentStateChanges.get(stateId);
                
                if (recentChange && (now - recentChange.timestamp) < 50) {
                    this.logger.debug(`Duplicate state change for ${stateId} within 50ms - skipping event`);
                    return;
                }
                
                // Clear existing timeout if any
                if (recentChange && recentChange.timeout) {
                    clearTimeout(recentChange.timeout);
                }
                
                // Track this state change
                const timeout = setTimeout(() => {
                    this.recentStateChanges.delete(stateId);
                }, 50);
                
                this.recentStateChanges.set(stateId, {
                    timestamp: now,
                    timeout: timeout
                });
                
                // Debug: Log how many subscriptions exist for this state
                if (this.serverSubscriptions.has(stateId)) {
                    const serverCount = this.serverSubscriptions.get(stateId).size;
                    this.logger.debug(`State ${stateId} has ${serverCount} server subscription(s) - forwarding event`);
                }
    ...
    formatResults(combinedResult) {
        ...
        for (const obj of payload) {
            if (obj.enumAssignments) {
                let hasAnyEnum = false;
                
                if (obj.enumAssignments.hasRoom) {
                    enumStatistics.objectsWithRooms++;
                    hasAnyEnum = true;
                }
                if (obj.enumAssignments.hasFunction) {
                    enumStatistics.objectsWithFunctions++;
                    hasAnyEnum = true;
                }
                
                // Count this object only once for "any enum", even if it has multiple enum types
                if (hasAnyEnum || obj.enumAssignments.totalEnums > 0) {
                    enumStatistics.objectsWithAnyEnum++;
                }
            ...
            if (obj.aliasInfo) {
                ...
                if (aliasTargetIds.has(obj._id) || (obj.aliasInfo.aliasedBy && obj.aliasInfo.aliasedBy.length > 0)) {
                    ...
                    isTargetObject = true;
                    // Count relationships from aliasedBy array (how many aliases point to this object)
                ...
                // Count objects with any alias relationship (avoid double counting)
