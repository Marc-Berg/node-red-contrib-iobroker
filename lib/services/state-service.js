...
class StateService {
    ...
    addSubscription(serverId, stateId) {
        ...
        if (this.activeSubscriptions.get(serverId).has(stateId)) {
            this.logger.debug(`Subscription for "${stateId}" on server ${serverId} already exists, emitting confirmation.`);
            // Even for existing subscriptions, emit confirmation so node status updates correctly
    ...
    handleConnectionSwitched(serverId) {
        ...
        this.handleServerReady(serverId, true); // Mark as reconnect to prevent initial values
    }
