...
class StateService {
    ...
    formatResults(combinedResult) {
        ...
        for (const [objectId, objectData] of Object.entries(objects)) {
                ...
                for (const enumRow of enums.rows) {
                    ...
                    if (enumObj && enumObj.common && enumObj.common.members && 
                        ...
                        if (enumInfo.type === 'rooms') {
                            ...
                            processedObject.enumAssignments.roomName = enumInfo.name;
                            ...
                            processedObject.enumAssignments.functionName = enumInfo.name;
                        } else {
            ...
            for (const [aliasId, aliasData] of Object.entries(aliases)) {
                ...
                if (pointsToThisObject) {
                    processedObject.aliasInfo.aliasedBy.push(aliasId);
                }
            }
        ...
        if (enums) {
            result.enums = enums;
            result.enumsProcessed = enums.rows ? enums.rows.length : 0;
        }
        
        if (aliases) {
            result.aliases = aliases;
            result.aliasesProcessed = Object.keys(aliases).length;
        }
