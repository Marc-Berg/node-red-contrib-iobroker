...
const IMMEDIATE_RETRY_BASE_DELAY_MS = 500;
const NORMAL_RETRY_BASE_DELAY_MS = 6000;
const MAX_BACKOFF_DELAY_MS = 60000;
const BACKOFF_MULTIPLIER = 1.5;
const BACKOFF_JITTER_MAX_MS = 2000;
const MAX_RETRY_ATTEMPTS = 30;
const BACKOFF_INCREMENT_MS = 1000;
const CLIENT_READY_TIMEOUT_MS = 5000;
const CLIENT_READY_POLL_INTERVAL_MS = 100;
const AUTH_ERROR_RETRY_ATTEMPTS = 3; // Limited retries for auth errors
const AUTH_ERROR_RETRY_DELAY_MS = 30000; // 30 seconds between auth error retries
...
class RecoveryManager {
    constructor(connectionManager) {
        ...
        this.authErrorAttempts = new Map(); // Track auth error retry attempts separately
    ...
    scheduleRetry(serverId, isImmediate = false) {
        ...
        const baseDelay = isImmediate ? IMMEDIATE_RETRY_BASE_DELAY_MS : NORMAL_RETRY_BASE_DELAY_MS;
        const backoffDelay = Math.min(baseDelay * Math.pow(BACKOFF_MULTIPLIER, attempts), MAX_BACKOFF_DELAY_MS);
        const jitterDelay = backoffDelay + (Math.random() * BACKOFF_JITTER_MAX_MS);
        ...
        const timer = setTimeout(async () => {
            ...
            if (![
                CONNECTION_STATES.RETRY_SCHEDULED,
                CONNECTION_STATES.NETWORK_ERROR,
    ...
    async waitForClientReady(client, serverId, timeout = CLIENT_READY_TIMEOUT_MS) {
        ...
        while (Date.now() - startTime < timeout) {
            ...
            await new Promise(resolve => setTimeout(resolve, CLIENT_READY_POLL_INTERVAL_MS));
        }
    ...
    handleRetryFailure(serverId, error) {
        ...
        // Check if this is an authentication error - handle with grace period
        if (ErrorClassifier.isAuthenticationError(errorMsg)) {
            this.log.debug(`Retry failed with authentication error for ${serverId}, delegating to auth error handler`);
        ...
        if (ErrorClassifier.isRetryableError(errorMsg)) {
            ...
            if (attempts < MAX_RETRY_ATTEMPTS) {
                ...
                }, BACKOFF_INCREMENT_MS * (attempts + 1));
    ...
    handleConnectionError(serverId, error) {
        ...
        if (ErrorClassifier.isAuthenticationError(errorMsg)) {
            const authAttempts = this.authErrorAttempts.get(serverId) || 0;
            
            if (authAttempts < AUTH_ERROR_RETRY_ATTEMPTS) {
                // Give user time to fix credentials on the server
                this.authErrorAttempts.set(serverId, authAttempts + 1);
                this.log.warn(`Authentication failed for ${serverId} (attempt ${authAttempts + 1}/${AUTH_ERROR_RETRY_ATTEMPTS}). Will retry in ${AUTH_ERROR_RETRY_DELAY_MS / 1000}s to allow credential fixes.`);
                ...
                const authRetryTimer = setTimeout(async () => {
                    this.retryTimers.delete(serverId);
                    if (this.getNodeCount(serverId) === 0) {
                        this.log.debug(`No nodes left for ${serverId} at auth retry time`);
                        this.clearRetryLock(serverId);
                        return;
                    }
                    
                    this.log.info(`Executing authentication retry for ${serverId}`);
                    await this.executeRetry(serverId);
                }, AUTH_ERROR_RETRY_DELAY_MS);
                
                this.retryTimers.set(serverId, authRetryTimer);
            } else {
                // Max auth retries reached - give up permanently
                this.connectionManager.setConnectionState(serverId, CONNECTION_STATES.AUTH_FAILED);
                this.log.error(`Authentication failed permanently for ${serverId} after ${AUTH_ERROR_RETRY_ATTEMPTS} attempts. Please check credentials.`);
                this.clearAllTimersForServer(serverId);
                this.clearRetryLock(serverId);
                this.authErrorAttempts.delete(serverId);
            }
    ...
    handleConnectionSuccess(serverId, client) {
        ...
        this.retryAttempts.set(serverId, 0);
        this.authErrorAttempts.delete(serverId); // Reset auth error counter on success
        this.clearAllTimersForServer(serverId);
        this.clearRetryLock(serverId);
    ...
    cleanupServerRecovery(serverId) {
        ...
        this.authErrorAttempts.delete(serverId); // Clean up auth error counter
    }
    ...
    cleanup() {
        ...
        this.authErrorAttempts.clear(); // Clear auth error attempts
