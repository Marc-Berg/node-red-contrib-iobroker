...
class WebSocketManager {
    constructor() {
        ...
        this._resubscribingServers = new Set();

        this.setupManagerInteractions();

        // Note: We do NOT register for global process events (uncaughtException, unhandledRejection)
        // because this would interfere with other Node-RED modules and plugins.
        // Instead, errors should be handled locally within our own promise chains.
    ...
    setupManagerInteractions() {
        // Give ConnectionManager reference to RecoveryManager for direct error delegation
        this.connectionManager.recoveryManager = this.recoveryManager;
        
        this.connectionManager.clearRecoveryCallbacksForServer = (serverId) => {
            this.nodeRegistry.clearRecoveryCallbacks(serverId);
        };

        this.connectionManager.cancelRecoveryForServer = (serverId) => {
            this.recoveryManager.cancelRetry(serverId);
            this.setRecoveryInProgress(serverId, false);
        };

        this.connectionManager.clearOperationQueueForServer = (serverId, error) => {
            this.operationManager.clearOperationQueue(serverId, error);
        };
    ...
    async handleClientReady(serverId, client) {
        ...
        if (this._resubscribingServers.has(serverId)) {
            this.log.info(`Skipping duplicate resubscription for ${serverId} - already in progress`);
            this.recoveryManager.handleConnectionSuccess(serverId, client);
            return;
        }

        this._resubscribingServers.add(serverId);
        ...
        setTimeout(async () => {
            try {
                ...
                // Clear per-server active/consolidated state so we rebuild subscriptions cleanly
                this.nodeRegistry.resetServerSubscriptions(serverId);
                ...
                if (client && client.sessionID) {
                    client.lastResubscriptionSessionID = client.sessionID;
                }
                ...
                this._resubscribingServers.delete(serverId);
            }
        }, 50);
    ...
    handleNodeUnregistered(serverId) {
        if (!serverId) {
            return;
        }
    ...
    unregisterFromEvents(nodeId) {
        this.nodeRegistry.unregisterFromEvents(nodeId);
    }
    ...
    async setState(serverId, stateId, value, ack = true, customTimestamp = null) {
        return await this.operationManager.setState(serverId, stateId, value, ack, customTimestamp);
    }

    async getStates(serverId, stateIds = null) {
        return await this.operationManager.getStates(serverId, stateIds);
    }
    ...
    async delObject(serverId, objectId, maintenance = false) {
        return await this.operationManager.delObject(serverId, objectId, maintenance);
    }

    async delObjects(serverId, objectIdOrPattern, maintenance = false) {
        return await this.operationManager.delObjects(serverId, objectIdOrPattern, maintenance);
    }
    ...
    async sendToHost(serverId, host, command, message, timeout = 10000) {
        return await this.operationManager.sendToHost(serverId, host, command, message, timeout);
    }
    ...
    removeAllEventListeners() {
        ...
        const { sigterm, sigint } = this.signalHandlers;

        if (sigterm) {
            process.off ? process.off('SIGTERM', sigterm) : process.removeListener('SIGTERM', sigterm);
        }

        if (sigint) {
            process.off ? process.off('SIGINT', sigint) : process.removeListener('SIGINT', sigint);
        }
    }
...
manager.signalHandlers = { sigterm: handleSigterm, sigint: handleSigint };
