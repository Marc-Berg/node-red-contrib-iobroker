...
class WebSocketManager {
    ...
    async getConnection(serverId, config) {
        ...
        if (oldHash && oldHash !== configHash) {
            ...
            this.nodeRegistry.clearInitialStatesSent(serverId);
        }
    ...
    async handleClientReady(serverId, client, connectionId) {
        ...
        setTimeout(async () => {
            if (hadRecoveryCallbacks) {
                ...
                await this.nodeRegistry.sendInitialValues(serverId, client);
            }
        }, 100);
    }
    ...
    startDeploy() {
        ...
        // Clear initial state cache at START of deploy
        this.nodeRegistry.initialStatesSent.clear();
        
        this.log(`Deploy #${this.deployCounter} started - initial state cache cleared`);
    ...
    registerNode(nodeId, serverId, type, skipDeploy = false) {
        return this.nodeRegistry.registerNode(nodeId, serverId, type, skipDeploy, this.deployCounter, this.isDeployActive);
    }
    ...
    async forceServerSwitch(oldServerId, newServerId, newConfig) {
        ...
        this.nodeRegistry.clearInitialStatesSent(oldServerId);
    ...
    getConnectionStatus(serverId) {
        ...
        // No connection info - check if we have recovery callbacks
        if (!client) {
            if (hasRecoveryCallbacks) {
                return {
                    ...
                    recoveryMode: true,
                    pendingRecoveryCallbacks: this.nodeRegistry.getRecoveryCallbackCount(serverId)
                };
            }
        ...
        return {
            ...
            pendingRecoveryCallbacks: hasRecoveryCallbacks ? this.nodeRegistry.getRecoveryCallbackCount(serverId) : 0
        };
    }
