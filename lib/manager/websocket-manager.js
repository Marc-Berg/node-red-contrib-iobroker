/*!
 * Improved WebSocket Manager with Operation Queueing
 * Handles operations during reconnect gracefully
 */
...
class WebSocketManager {
    constructor() {
        ...
        this.operationQueues = new Map();      // NEW: Queue operations during reconnect
        this.queueProcessors = new Map();      // NEW: Queue processing timeouts
    ...
    scheduleRetry(serverId) {
        ...
        const timer = setTimeout(async () => {
            ...
            if (currentNodes > 0 && this.getConnectionState(serverId) === CONNECTION_STATES.RETRY_SCHEDULED) {
                this.log(`Attempting retry for ${serverId}`);
                this.setConnectionState(serverId, CONNECTION_STATES.IDLE);
            }
        }, retryDelay);
    ...
    async cleanup() {
        ...
        this.storedConfigs.clear(); // Clear stored configs on shutdown
