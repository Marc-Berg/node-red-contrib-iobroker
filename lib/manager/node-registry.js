...
class NodeRegistry {
    ...
    async resubscribeStates(serverId, client) {
        ...
        if (statesToSubscribe.size > 0) {
            this.manager.log.info(`Resubscribing to ${statesToSubscribe.size} patterns for ${serverId}`);
            ...
            for (const pattern of statesToSubscribe) {
                try {
                    ...
                    this.manager.log.info(`Resubscribed to pattern: ${pattern}`);

                    // Small delay between subscriptions to avoid overwhelming the server
                    await new Promise(r => setTimeout(r, 50));
                } catch (error) {
                    this.manager.log.error(`Resubscribe failed for ${pattern}: ${error.message}`);
                }
            }
        }
