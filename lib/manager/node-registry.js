                ...
                try {
                    ...
                    if (callbackInfos) {
                        callbackInfos.forEach(info => {
                            ...
                            // Send initial value for single states (not wildcards)
                            if (info.callback.wantsInitialValue && !pattern.includes('*')) {
                                setImmediate(async () => {
                                    try {
                                        const state = await client.getState(pattern);
                                        if (state && state.val !== undefined) {
                                            this.manager.log(`Reconnect initial value for node ${info.nodeId}: ${pattern} = ${state.val}`);
                                            
                                            if (info.callback.onInitialValue) {
                                                info.callback.onInitialValue(pattern, state);
                                            } else {
                                                info.callback(pattern, state);
                                            }
                                        }
                                    } catch (error) {
