...
class NodeRegistry {
    ...
    async subscribeMultiple(nodeId, serverId, stateIds, callback, config, isRecovery = false, manager) {
        try {
            ...
            if (connectionStatus.ready && isClientReady) {
                try {
                    // Parallel subscription with concurrency limit
                    ...
                    // Optimized initial value loading
    ...
    async loadInitialValuesParallel(nodeId, serverId, stateIds, callback, client) {
        try {
            ...
            // Load initial values in parallel batches
            const batchSize = 15;
            const batches = [];
    ...
    async resubscribeStates(serverId, client) {
        ...
        // Parallel resubscription for single patterns
        ...
        // Optimized resubscription for multiple state nodes
        ...
        if (multipleStateNodes.size > 0) {
            ...
            // Collect all unique states from all multiple state nodes
            ...
            // Subscribe to all unique states in parallel
            ...
            // Update each node's subscription info
