            ...
            for (const patt of Array.from(active)) {
                if (!desired.has(patt)) {
                    try {
                        ...
                        active.delete(patt);
                        // Also reflect in consolidated set bookkeeping
                        ...
                        this.log.error(`Failed to unsubscribe obsolete pattern ${patt}: ${e.message}`);
                    }
                }
            }
            ...
            if (nodeIds.size === 0) {
                ...
                if (remaining === 0) {
                    try {
                        ...
                        this.log.error(`Unsubscribe error for consolidated ${consolidatedTarget}: ${e.message}`);
                    }
                } else {
    ...
    async unsubscribeMultiple(nodeId, serverId, stateIds, manager) {
        try {
            ...
            for (const stateId of subscribedStates) {
                const consolidatedTarget = mapping.get(stateId) || stateId;
            ...
            for (const [target] of targetsToCheck) {
                ...
                if (remaining === 0) {
                    try {
                        ...
                        this.log.error(`Error unsubscribing consolidated ${target}: ${e.message}`);
                    }
                } else {
