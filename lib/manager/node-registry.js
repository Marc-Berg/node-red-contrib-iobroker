...
class NodeRegistry {
    ...
    getRecoveryCallbackCount(serverId) {
        const callbacks = this.recoveryCallbacks.get(serverId);
    ...
    registerNode(nodeId, serverId, type, skipDeploy = false, deployCounter, isDeployActive) {
        if (!skipDeploy && !isDeployActive) {
            this.manager.startDeploy();
        }
        ...
        if (!skipDeploy && this.manager.deployTimeout) {
            clearTimeout(this.manager.deployTimeout);
            this.manager.deployTimeout = setTimeout(() => {
                this.manager.completeDeploy();
            }, 2000);
        }
    }
    ...
    async subscribe(nodeId, serverId, stateIdOrPattern, callback, config, isRecovery = false, manager) {
        try {
            ...
            this.registerNode(nodeId, serverId, 'subscribe', isRecovery, manager.deployCounter, manager.isDeployActive);
            ...
            if (client.isClientReady()) {
                ...
                // Send initial value if requested - schedule it to run after subscription is complete
                        ...
                        try {
                            ...
                            if (!sentStates.has(nodeStateKey)) {
                                ...
                                if (state && state.val !== undefined) {
                                    // Mark state as initial value and send with delay to ensure Node-RED is ready
                                    setTimeout(() => {
                                        ...
                                        this.manager.log(`Initial value sent for node ${nodeId} state ${stateIdOrPattern}: ${state.val}`);
                                    }, 100); // Small delay to ensure Node-RED is ready
                                } else {
                                    this.manager.log(`No initial value available for node ${nodeId} state ${stateIdOrPattern}`);
                                }
                            } else {
                                this.manager.log(`Initial value already sent for node ${nodeId} state ${stateIdOrPattern}`);
                            }
                        } catch (error) {
                            this.manager.error(`Initial value failed for node ${nodeId} state ${stateIdOrPattern}: ${error.message}`);
                        }
                    });
                }
            }
    ...
    async registerForEvents(nodeId, serverId, callback, config, isRecovery = false, manager) {
        try {
            // Skip deploy trigger during recovery
            this.registerNode(nodeId, serverId, 'events', isRecovery, manager.deployCounter, manager.isDeployActive);
    ...
    async sendInitialValues(serverId, client) {
        ...
        if (nodesWantingInitial.size === 0) {
            this.manager.log(`No nodes requiring initial values for ${serverId}`);
            return;
        }

        this.manager.log(`Sending initial values for ${nodesWantingInitial.size} states on ${serverId}`);
                    ...
                    nodeIds.forEach(nodeId => {
                        ...
                        if (!sentStates.has(nodeStateKey)) {
                            ...
                            if (callback) {
                                callback(stateId, state);
                                sentStates.add(nodeStateKey);
                                this.manager.log(`Initial value sent for node ${nodeId} state ${stateId}: ${state.val}`);
                            }
                        } else {
                            this.manager.log(`Initial value already sent for node ${nodeId} state ${stateId}, skipping`);
                        }
                    });
                } else {
                    this.manager.log(`No initial value available for state ${stateId}`);
                }
                
                await new Promise(r => setTimeout(r, 25));
            } catch (error) {
                this.manager.error(`Failed to get initial value for ${stateId}: ${error.message}`);
            }
        }
    }

    clearInitialStatesSent(serverId) {
