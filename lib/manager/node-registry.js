...
class NodeRegistry {
    ...
    // Helper to get mapping container per server
    ...
    async unsubscribe(nodeId, serverId, stateIdOrPattern, manager) {
        try {
            // If registry is being destroyed or core collections are unavailable, do minimal cleanup and exit
            if (this.destroyed || !this.subscriptions || !this.callbacks || !this.nodeRegistrations) {
                this.callbacks && this.callbacks.delete(nodeId);
                try { this.unregisterNode(nodeId); } catch (_) {}
                this.log.debug(`Skipping unsubscribe for node ${nodeId} during shutdown`);
                return;
            }
            ...
            if (nodeIds.size === 0) {
                ...
                const mapping = this.patternMapping ? this.getPatternMapping(serverId) : null;
                const consolidatedTarget = mapping ? (mapping.get(stateIdOrPattern) || stateIdOrPattern) : stateIdOrPattern;

                // Remove mapping for this requested pattern
                if (mapping) mapping.delete(stateIdOrPattern);

                // If no other requested patterns map to this consolidated target, drop WS subscription
                const remaining = this.patternMapping ? this.countRequestsForConsolidated(serverId, consolidatedTarget) : 0;
                if (remaining === 0) {
                    const ready = manager && manager.connectionManager && manager.connectionManager.isConnectionReady(serverId);
                    if (ready) {
                        try {
    ...
    async unsubscribeMultiple(nodeId, serverId, stateIds, manager) {
        try {
            // If registry is being destroyed or core collections are unavailable, do minimal cleanup and exit
            if (this.destroyed || !this.multipleStateSubscriptions || !this.callbacks || !this.nodeRegistrations) {
                this.multipleStateSubscriptions && this.multipleStateSubscriptions.delete(nodeId);
                this.callbacks && this.callbacks.delete(nodeId);
                try { this.unregisterNode(nodeId); } catch (_) {}
                this.log.debug(`Skipping multiple unsubscribe for node ${nodeId} during shutdown`);
                return;
            }
            ...
            for (const stateId of subscribedStates) {
                const consolidatedTarget = mapping ? (mapping.get(stateId) || stateId) : stateId;
                // Remove the mapping for this requested state
                if (mapping) mapping.delete(stateId);
            ...
            for (const [target] of targetsToCheck) {
                const remaining = this.patternMapping ? this.countRequestsForConsolidated(serverId, target) : 0;
                if (remaining === 0) {
                    const ready = manager && manager.connectionManager && manager.connectionManager.isConnectionReady(serverId);
                    if (ready) {
                        try {
