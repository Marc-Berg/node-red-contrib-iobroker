...
class NodeRegistry {
    ...
    registerPendingInitialValue(serverId, nodeId, stateId) {
        ...
        this.manager.log(`Registered pending initial value for node ${nodeId} state ${stateId}`);
    }
    ...
    async processPendingInitialValues(serverId, client) {
        ...
        this.manager.log(`Processing ${pending.size} pending initial values for ${serverId}`);
        ...
        for (const [nodeId, stateId] of pendingEntries) {
            try {
                ...
                this.manager.log(`Fetching initial value for node ${nodeId} state ${stateId}`);

                const state = await client.getState(stateId);
                if (state && state.val !== undefined) {
                    this.manager.log(`Initial value received for node ${nodeId}: ${stateId} = ${state.val}`);
                    ...
                    this.manager.log(`No initial value available for node ${nodeId} state ${stateId}`);
                }
                ...
                this.manager.error(`Initial value request failed for node ${nodeId} state ${stateId}: ${error.message}`);
            }
        }
    ...
    async requestInitialValue(nodeId, serverId, stateId, callback, client) {
        try {
            this.manager.log(`Requesting initial value for node ${nodeId} state ${stateId}`);
    ...
    cleanup() {
        ...
        this.pendingInitialValues.clear(); // FIXED: Clean up pending initial values
