...
const { Logger, PatternMatcher } = require('../utils');
...
class NodeRegistry {
    ...
    async processPendingInitialValues(serverId, client) {
        ...
        for (const [nodeId, stateData] of pendingEntries) {
            try {
                ...
                if (Array.isArray(stateData)) {
                    for (const stateId of stateData) {
                        await this.fetchAndDeliverInitialValue(nodeId, stateId, callback, client);
    ...
    async subscribeMultiple(nodeId, serverId, stateIds, callback, config, isRecovery = false, manager) {
        try {
                ...
                try {
                    for (const stateId of stateIds) {
                        try {
                            ...
                            this.log.error(`Failed to subscribe to state ${stateId}: ${subscribeError.message}`);
                    ...
                    if (callback.wantsInitialValue && successfulStates.length > 0) {
                        for (const stateId of successfulStates) {
                            await this.requestInitialValue(nodeId, serverId, stateId, callback, client);
                            await new Promise(resolve => setTimeout(resolve, 50));
                        }
                    }
                } catch (subscribeError) {
            ...
            this.log.info(`Subscribed node ${nodeId} to ${successfulStates.length}/${stateIds.length} multiple states`);
    ...
    async subscribeObjects(nodeId, serverId, objectIdOrPattern, callback, config, isRecovery = false, manager) {
        try {
            ...
            this.log.info(`Subscribed node ${nodeId} to ${objectIdOrPattern.includes('*') ? 'wildcard pattern' : 'single object'}: ${objectIdOrPattern}`);
    ...
    syncAllNodeStatuses(serverId) {
        ...
        this.log.info(`Force-syncing all node statuses for ${serverId} to: ${nodeStatus}`);
    ...
    async unsubscribeMultiple(nodeId, serverId, stateIds, manager) {
        try {
            ...
            this.log.info(`Unsubscribed node ${nodeId} from ${subscribedStates.length} multiple states`);
    ...
    async unsubscribeObjects(nodeId, serverId, objectIdOrPattern, manager) {
        try {
            ...
            this.log.info(`Unsubscribed node ${nodeId} from object pattern ${objectIdOrPattern}`);
        ...
        if (multipleStateNodes.size > 0) {
            ...
            for (const [nodeId, stateInfo] of multipleStateNodes) {
                try {
                    ...
                    for (const stateId of stateInfo.originalStates) {
                        try {
                            await this.manager.operationManager.subscribe(serverId, stateId, (id, state) => this.handleStateChange(id, state));
                            successfulStates.push(stateId);
                        } catch (subscribeError) {
                            this.log.error(`Failed to resubscribe to state ${stateId}: ${subscribeError.message}`);
                        }
                    }

                    stateInfo.subscribedStates = new Set(successfulStates);
                    ...
                    this.log.debug(`Resubscribed multiple state node ${nodeId} to ${successfulStates.length}/${stateInfo.originalStates.length} states`);

                    await new Promise(r => setTimeout(r, 100));
                } catch (error) {
                    this.log.error(`Resubscribe failed for multiple state node ${nodeId}: ${error.message}`);
                }
            }
        }
    ...
    async subscribeToLogs(nodeId, serverId, callback, config, logLevel = 'info', isRecovery = false, manager) {
        try {
            ...
            this.log.info(`Subscribed node ${nodeId} to live logs (level: ${logLevel})`);
    ...
    async unsubscribeFromLogs(nodeId, serverId, manager) {
        try {
            ...
            this.log.info(`Unsubscribed node ${nodeId} from live logs`);
