...
class NodeRegistry {
    ...
    async subscribeMultiple(nodeId, serverId, stateIds, callback, config, isRecovery = false, manager) {
        try {
            ...
            const isAuthError = error.message.includes('Authentication failed') || error.message.includes('not possible in state: auth_failed');
            const isNetworkError = connectionState === 'network_error' || connectionState === 'retry_scheduled';
            ...
            if (!isAuthError && !isNetworkError) {
                ...
                this.log.debug(`Multiple subscribe skipped for node ${nodeId} (connection state: ${connectionState})`);
            }

            if (callback.updateStatus) {
                if (isAuthError || connectionState === 'auth_failed') {
    ...
    async subscribe(nodeId, serverId, stateIdOrPattern, callback, config, isRecovery = false, manager) {
        try {
            ...
            if (!isAuthError && !isNetworkError) {
                ...
                this.log.debug(`Subscribe skipped for node ${nodeId} (connection state: ${connectionState})`);
            }
    ...
    async fetchInitialValueWithTimeout(nodeId, stateId, callback, client) {
        ...
        try {
            const timeoutPromise = new Promise((_, reject) => {
                timeoutId = setTimeout(() => reject(new Error('Initial value timeout')), 2000);
            });
                    ...
                    if (consolidatedPattern === pattern) {
                        ...
                        if (callbackInfos) {
                            callbackInfos.forEach(info => {
                                if (notifiedNodeIds.has(info.nodeId)) {
                                    return;
                                }
                                notifiedNodeIds.add(info.nodeId);
    ...
    registerNode(nodeId, serverId, type) {
        const existingRegistration = this.nodeRegistrations.get(nodeId);
        ...
        if (!existingRegistration) {
            if (this.manager && this.manager.recoveryManager) {
                this.manager.recoveryManager.incrementNodeCount(serverId);
            }
        } else if (existingRegistration.serverId !== serverId) {
            // Node moved to a different server — adjust counters on both sides.
            // Use decrementNodeCount directly (not handleNodeUnregistered) to avoid
            // triggering connection-close logic for the old server, which was already
            // cleaned up by forceCleanupConnection during the config change.
            if (this.manager && this.manager.recoveryManager) {
                this.manager.recoveryManager.decrementNodeCount(existingRegistration.serverId);
                this.manager.recoveryManager.incrementNodeCount(serverId);
            }
            this.log.debug(`Node ${nodeId} re-registered from ${existingRegistration.serverId} to ${serverId}`);
        }
    ...
    unregisterNode(nodeId) {
        ...
        if (registration) {
            ...
            const pendingForServer = (this.pendingInitialValues && this.pendingInitialValues.get)
                ? this.pendingInitialValues.get(serverId)
                : null;
            if (pendingForServer) {
            ...
            if (this.manager && typeof this.manager.handleNodeUnregistered === 'function') {
                this.manager.handleNodeUnregistered(serverId);
            }

            this.log.debug(`Unregistered node ${nodeId} from ${serverId}`);
            return serverId;
        }
        return null;
    }
        ...
        const processingTasks = pendingEntries.map(async ([nodeId, stateData]) => {
            try {
                ...
                if (Array.isArray(stateData)) {
                    ...
                    await this.fetchInitialValueWithTimeout(nodeId, stateData, callback, client);
                }
            } catch (error) {
    ...
    async subscribeObjects(nodeId, serverId, objectIdOrPattern, callback, config, isRecovery = false, manager) {
        try {
            ...
            if (!isAuthError && !isNetworkError) {
                ...
                this.log.debug(`Object subscribe skipped for node ${nodeId} (connection state: ${connectionState})`);
            }
    ...
    async subscribeToLogs(nodeId, serverId, callback, config, logLevel = 'info', isRecovery = false, manager) {
        try {
            ...
            if (!isAuthError && !isNetworkError) {
                ...
                this.log.debug(`Log subscribe skipped for node ${nodeId} (connection state: ${connectionState})`);
            }
