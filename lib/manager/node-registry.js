...
class NodeRegistry {
    ...
    async subscribe(nodeId, serverId, stateIdOrPattern, callback, config, isRecovery = false, manager) {
        try {
            ...
            this.manager.log.info(`Node ${nodeId} registration - Connection ready: ${connectionStatus.ready}, Client ready: ${isClientReady}`);

            if (connectionStatus.ready && isClientReady) {
                // Sofortige Subscription und Status-Update
                try {
                    ...
                    if (callback.updateStatus) {
                        ...
                        this.manager.log.info(`Node ${nodeId} immediately set to ready status`);
                    }
                    ...
                    this.manager.log.error(`Immediate subscription failed for ${nodeId}: ${subscribeError.message}`);
                    throw subscribeError;
                }
            } else {
                if (callback.updateStatus) {
                    callback.updateStatus('connecting');
                    this.manager.log.info(`Node ${nodeId} set to connecting status - will get ready update later`);
                }

                // Initial value für später registrieren
                if (callback.wantsInitialValue && !stateIdOrPattern.includes('*')) {
            ...
            // Recovery scheduling only for retryable errors
    ...
    syncAllNodeStatuses(serverId) {
        ...
        this.manager.log.info(`Force-syncing all node statuses for ${serverId} to: ${nodeStatus}`);

        // Force-Update aller Nodes für diesen Server
        let updatedCount = 0;
            ...
            if (registration && registration.serverId === serverId) {
                if (callback.updateStatus) {
                    try {
                        ...
                        this.manager.log.error(`Force sync error for node ${nodeId}: ${err.message}`);
                    }
                }
            }
        });
            ...
            if (registration && registration.serverId === serverId) {
                if (callback.updateStatus) {
                    try {
                        ...
                        this.manager.log.error(`Force sync error for object node ${nodeId}: ${err.message}`);
                    }
                }
            }
        });
            ...
            if (registration && registration.serverId === serverId) {
                if (callback.updateStatus) {
                    try {
                        ...
                        this.manager.log.error(`Force sync error for event node ${nodeId}: ${err.message}`);
                    }
                }
            }
        });

        this.manager.log.info(`Force-synced ${updatedCount} nodes for ${serverId}`);
        return updatedCount;
    }
    ...
    async subscribeToLogs(nodeId, serverId, callback, config, logLevel = 'info', isRecovery = false, manager) {
        try {
            ...
            // Subscribe to logs via WebSocket if client is ready
            if (client.isClientReady()) {
            ...
            this.manager.log.info(`Subscribed node ${nodeId} to live logs (level: ${logLevel})`);

        } catch (error) {
            this.manager.log.error(`Log subscribe failed for node ${nodeId}: ${error.message}`);
            ...
            if (!isRecovery && !error.message.includes('auth_failed') && !error.message.includes('Authentication failed')) {
                const recoveryCallback = () => {
                    this.manager.log.info(`Attempting recovery log subscription for node ${nodeId}`);
                    this.subscribeToLogs(nodeId, serverId, callback, config, logLevel, true, manager)
                        .catch(retryError => {
                            this.manager.log.error(`Recovery log subscription failed for node ${nodeId}: ${retryError.message}`);
                        });
                };
    ...
    async unsubscribeFromLogs(nodeId, serverId, manager) {
        try {
            // Check if this was the last log subscriber
            const remainingLogNodes = this.getRemainingLogNodesForServer(serverId);

            if (remainingLogNodes <= 1) {
                // This is the last log subscriber, unsubscribe from WebSocket
                const client = manager.connectionManager.connections.get(serverId);
                if (client && client.connected) {
                    await manager.operationManager.unsubscribeFromLogs(serverId);
                    this.manager.log.info(`Unsubscribed from WebSocket logs for ${serverId}`);
                }
            }
            ...
            this.manager.log.info(`Unsubscribed node ${nodeId} from live logs`);

        } catch (error) {
            this.manager.log.error(`Log unsubscribe error for node ${nodeId}: ${error.message}`);
        }
    }

    // Handle log messages from WebSocket
    handleLogMessage(logData) {
        // Forward log messages to all registered log callbacks
        this.callbacks.forEach((callback, nodeId) => {
            const registration = this.nodeRegistrations.get(nodeId);
            if (registration && registration.type === 'subscribeToLogs') {
                try {
                    ...
                    this.manager.log.error(`Log callback error for ${nodeId}: ${err.message}`);
                }
            }
        });
    }

    // Helper method to count remaining log nodes for a server
    ...
    // Resubscribe to logs after reconnection
    async resubscribeLogs(serverId, client) {
        const logNodesToResubscribe = new Map();

        // Collect all log nodes that need resubscription for this server
        ...
        if (logNodesToResubscribe.size > 0) {
            this.manager.log.info(`Resubscribing to logs for ${logNodesToResubscribe.size} nodes on ${serverId}`);

            try {
                // Resubscribe to logs (assuming 'info' level for now - could be enhanced to store per-node levels)
                await this.manager.operationManager.subscribeToLogs(serverId, 'info', (logData) => this.handleLogMessage(logData));

                // Notify all log nodes that subscription is restored
                ...
                this.manager.log.info(`Resubscribed to logs successfully`);

            } catch (error) {
                this.manager.log.error(`Resubscribe to logs failed: ${error.message}`);
            }
        }
    }
