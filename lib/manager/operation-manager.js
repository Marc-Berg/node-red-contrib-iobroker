...
class OperationManager {
    ...
    async getObjects(serverId, pattern = '*', objectType = null) {
        return await this.executeOperation(serverId, async () => {
            ...
            // Single object (no wildcards)
            if (pattern && !pattern.includes('*')) {
                return await this.getSingleObject(serverId, pattern, objectType);
            }

            // Specific type - use getObjectView
            if (objectType) {
                this.log.debug(`Using getObjectView for type: ${objectType}`);
                return await this.getObjectsViaObjectView(serverId, pattern, objectType);
            }

            // All types - use getObjectView for all known types
            this.log.debug(`Using getObjectView for all types`);
            return await this.getObjectsAllTypes(serverId, pattern);
        }, `getObjects(${pattern}${objectType ? `, type: ${objectType}` : ''})`);
    }

    async getSingleObject(serverId, objectId, objectType) {
        const client = this.connectionManager.connections.get(serverId);
        ...
        return this.timeoutHandler.withTimeout(
                ...
                client.emit('getObject', objectId, (err, obj) => {
                    if (err) {
                        ...
                        if (objectType && obj && obj.type !== objectType) {
                            result = [];
                            this.log.debug(`Single object ${objectId} filtered out: found type '${obj.type}', wanted '${objectType}'`);
                        } else if (obj) {
                            this.log.debug(`Single object ${objectId} matches: type '${obj.type}'`);
                        }
            ...
            10000,
            'Get single object timeout'
        );
    }

    async getObjectsAllTypes(serverId, pattern) {
        const client = this.connectionManager.connections.get(serverId);
        ...
        const promises = allTypes.map(objectType => 
            this.timeoutHandler.withTimeout(
                new Promise((resolve) => {
                    client.emit('getObjectView', 'system', objectType, viewParams, (err, result) => {
                        if (err) {
                            this.log.debug(`getObjectView for ${objectType} failed: ${err}`);
                            resolve([]);
                        } else {
                            const objects = [];
                            ...
                            this.log.debug(`getObjectView(${objectType}): ${objects.length} objects`);
                            resolve(objects);
                        }
                    });
                }),
                10000,
                `getObjectView(${objectType}) timeout`
            )
        );
    ...
    async getObjectsViaObjectView(serverId, pattern, objectType) {
        const client = this.connectionManager.connections.get(serverId);
