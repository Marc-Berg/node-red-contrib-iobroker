...
class OperationManager {
    ...
    async executeOperation(serverId, operation, operationName = 'operation') {
        if (this.destroyed || this.connectionManager.destroyed) {
            this.log.debug(`Skipping ${operationName} for ${serverId} - manager destroyed`);
            return;
        }

        const isReady = this.connectionManager.isConnectionReady(serverId);
        const isUnsubscribeOp = /unsubscribe/i.test(operationName);

        // Best-effort fast path: don't queue or reconnect for unsubscribe-like operations
        // If the connection isn't ready, just return without error to avoid close timeouts
        if (!isReady && isUnsubscribeOp) {
            const state = this.connectionManager.getConnectionState(serverId);
            this.log.debug(`Skipping ${operationName} for ${serverId} - connection not ready (state: ${state})`);
            return;
        }
        ...
        if (isReady) {
            try {
                ...
                if ((operationName.includes('unsubscribe') || operationName.includes('Unsubscribe')) && 
                    error.message && error.message.includes('timeout') &&
                    (this.destroyed || this.connectionManager.destroyed)) {
                    this.log.debug(`${operationName} timeout during shutdown for ${serverId}, ignoring.`);
                    return;
                }
    ...
    async setState(serverId, stateId, value, ack = true, customTimestamp = null) {
        return await this.executeOperation(serverId, async () => {
            const client = this.connectionManager.connections.get(serverId);
            if (!client || !client.isClientReady()) {
                throw new Error(`No ready connection for ${serverId}`);
            ...
            return new Promise((resolve, reject) => {
                ...
                const state = { val: value, ack, from: 'system.adapter.node-red', ts: timestamp };
    ...
    async getStates(serverId, stateIds = null) {
        return await this.executeOperation(serverId, async () => {
            const client = this.connectionManager.connections.get(serverId);
            if (!client || !client.isClientReady()) {
                throw new Error(`No ready connection for ${serverId}`);
            ...
            if (stateIds && Array.isArray(stateIds)) {
                // Bulk mode: get specific states
                const results = {};
                await Promise.all(stateIds.map(async (stateId) => {
                    try {
                        results[stateId] = await client.getState(stateId);
                    } catch (error) {
                        this.log.warn(`Failed to get state ${stateId}: ${error.message}`);
                        results[stateId] = null;
                    }
                }));
                return results;
            } else {
                // Legacy mode: get all states
        ...
        }, stateIds ? `getStates([${stateIds.length} states])` : 'getStates');
    }
        ...
        if (result && result.rows) {
            for (const row of result.rows) {
                if (row.value && row.value.type === objectType) {
                    ...
                    if (PatternMatcher.matches(row.id, pattern)) {
    ...
    filterObjectsByPattern(allObjects, pattern, objectType = null) {
        if (pattern === '*') {
            return this.filterObjectsByType(allObjects, objectType);
        }

        const filtered = {};
        if (allObjects && typeof allObjects === 'object') {
            for (const [objectId, objectData] of Object.entries(allObjects)) {
                if (objectData && PatternMatcher.matches(objectId, pattern)) {
                    filtered[objectId] = objectData;
                }
            }
        }

        if (objectType) {
            this.log.debug(`Pattern filtering matched ${Object.keys(filtered).length} objects for pattern '${pattern}'`);
        }

        return this.filterObjectsByType(filtered, objectType);
    }
    ...
    async delObject(serverId, objectId, maintenance = false) {
        return await this.executeOperation(serverId, async () => {
            const client = this.connectionManager.connections.get(serverId);
            if (!client || !client.isClientReady()) {
                throw new Error(`No ready connection for ${serverId}`);
            ...
            const options = maintenance ? { maintenance: true } : {};

            return this.timeoutHandler.withTimeout(
                new Promise((resolve, reject) => {
                    client.emit('delObject', objectId, options, (err) => {
                        if (err) {
                            reject(new Error(err));
                        } else {
                            resolve();
    ...
    async delObjects(serverId, objectIdOrPattern, maintenance = false) {
        return await this.executeOperation(serverId, async () => {
            const client = this.connectionManager.connections.get(serverId);
            if (!client || !client.isClientReady()) {
                throw new Error(`No ready connection for ${serverId}`);
            ...
            return this.timeoutHandler.withTimeout(
                new Promise((resolve, reject) => {
                    client.emit('delObjects', objectIdOrPattern, options, (err) => {
                        if (err) {
                            reject(new Error(err));
                        } else {
                            resolve();
                ...
                15000,
                'Delete objects timeout'
            );
        }, `delObjects(${objectIdOrPattern})`);
    }
    ...
    async unsubscribe(serverId, stateIdOrPattern) {
        return await this.executeOperation(serverId, async () => {
            const client = this.connectionManager.connections.get(serverId);
            if (!client || !client.isClientReady() || client.destroyed || !client.connected || !client.socket) {
                this.log.debug(`Skipping unsubscribe for ${serverId} - client not available or destroyed`);
                return;
            }
            ...
            return this.timeoutHandler.withTimeout(
                new Promise((resolve) => {
                    try {
                        ...
                        this.log.debug(`Unsubscribe emit failed for ${serverId}: ${error.message}`);
                        resolve();
                    }
                }),
                2000,
    ...
    async unsubscribeObjects(serverId, objectIdOrPattern) {
        return await this.executeOperation(serverId, async () => {
            const client = this.connectionManager.connections.get(serverId);
            if (!client || !client.isClientReady() || client.destroyed || !client.connected || !client.socket) {
                this.log.debug(`Skipping object unsubscribe for ${serverId} - client not available or destroyed`);
                return;
            }
            ...
            return this.timeoutHandler.withTimeout(
                new Promise((resolve) => {
                    try {
                        ...
                        this.log.debug(`Unsubscribe objects emit failed for ${serverId}: ${error.message}`);
                        resolve();
                    }
                }),
                2000,
                ...
                new Promise((resolve) => {
                    try {
                        client.emit('requireLog', false, () => {
                            if (client.handlers && client.handlers['log']) {
                                client.handlers['log'] = [];
                            }
                            resolve();
                        });
                    } catch (error) {
                        this.log.debug(`Log unsubscribe emit failed for ${serverId}: ${error.message}`);
                        resolve();
                    }
                }),
                3000,
    ...
    async sendToHost(serverId, host, command, message, timeout = 10000) {
        return await this.executeOperation(serverId, async () => {
            const client = this.connectionManager.connections.get(serverId);
            if (!client || !client.isClientReady()) {
                throw new Error(`No ready connection for ${serverId}`);
            ...
            return new Promise((resolve, reject) => {
                if (timeout === null) {
                    try {
                        client.emit('sendToHost', host, command, message);
                        this.log.debug(`Fire-and-forget sendToHost: ${host}.${command}`);
                        resolve(null);
                    } catch (error) {
                        reject(error);
                    }
                    ...
                    const timeoutHandle = setTimeout(() => {
                        reject(new Error(`sendToHost timeout after ${timeout}ms`));
                    }, timeout);

                    const callback = (result) => {
                        clearTimeout(timeoutHandle);
                        this.log.debug(`sendToHost response received from ${host}: ${JSON.stringify(result).substring(0, 200)}`);
                        resolve(result);
                    };

                    try {
                        client.emit('sendToHost', host, command, message, callback);
                        this.log.debug(`sendToHost with response: ${host}.${command} - waiting for response`);
                    } catch (error) {
                        clearTimeout(timeoutHandle);
                        reject(error);
                    }
        ...
        }, `sendToHost(${host}${command ? `, ${command}` : ''})`);
    }
