...
class OperationManager {
    ...
    async getBulkStates(serverId, stateIds) {
        return await this.executeOperation(serverId, async () => {
            const client = this.connectionManager.connections.get(serverId);
            if (!client || !client.isClientReady()) {
                throw new Error(`No ready connection for ${serverId}`);
            ...
            await Promise.all(stateIds.map(async (stateId) => {
                try {
                    ...
                    this.logger.warn(`Failed to get state ${stateId}: ${error.message}`);
                    results[stateId] = null;
                }
            }));
            
            return results;
        }, `getBulkStates([${stateIds.length} states])`);
    }
    ...
    async setState(serverId, stateId, value, ack = true, customTimestamp = null) {
        return await this.executeOperation(serverId, async () => {
            ...
            return new Promise((resolve, reject) => {
                ...
                const state = { val: value, ack, from: 'system.adapter.node-red', ts: timestamp, lc: Date.now() };
