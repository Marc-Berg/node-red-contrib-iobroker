...
class ConnectionManager {
    ...
    setConnectionState(serverId, state) {
        ...
        const nodeCountMsg = nodeCount === 'unknown' ? 'awaiting node registration' : `${nodeCount} nodes affected`;
        this.log.info(`Connection state changed for ${serverId}: ${oldState || 'undefined'} -> ${state} (${nodeCountMsg})`);
    ...
    async getConnection(serverId, config) {
        ...
        // Clean up any stale serverIds from the same config node
        const currentConfigId = config.id; // The config node ID
        
        for (const [storedServerId, storedConfig] of this.storedConfigs.entries()) {
            if (storedServerId !== serverId && storedConfig.id === currentConfigId) {
                this.log.info(`Detected configuration change from ${storedServerId} to ${serverId} - cleaning up old connection`);
                
                if (this.clearOperationQueueForServer) {
                    this.log.info(`Clearing operation queue for old serverId ${storedServerId}`);
                    this.clearOperationQueueForServer(storedServerId, new Error(`Configuration changed - new serverId ${serverId}`));
                }
                
                await this.forceCleanupConnection(storedServerId);
                this.storedConfigs.delete(storedServerId);
            }
        }
    ...
    async attemptReconnection(serverId) {
        ...
        try {
            ...
            if (client && client.connected !== false) {
                ...
                this.log.debug(`Reconnection to ${serverId} deferred to recovery (connection already in retry state)`);
                return null;
            }
    ...
    handleConnectionError(serverId, error) {
        ...
        // Prefer a meaningful message; if missing, fall back to name/inspection
        let errorMsg = '';
        try {
            if (error && error.message) errorMsg = error.message;
            else if (error && error.name) errorMsg = error.name;
            else errorMsg = String(error);
        } catch (e) {
            errorMsg = `Uninspectable error: ${String(error)}`;
        }

        this.log.error(`Connection error for ${serverId}: ${errorMsg}`);

        // If the error message is empty or unhelpful, log the full object/stack at debug level
        if (!errorMsg || String(errorMsg).trim() === '') {
            try {
                this.log.debug(`Full connection error object for ${serverId}: ${error && error.stack ? error.stack : JSON.stringify(error)}`);
            } catch (e) {
                this.log.debug(`Connection error inspect failed: ${e && e.message ? e.message : String(e)}`);
            }
        }

        // Only delegate to RecoveryManager during initial connection attempt
        // Errors from existing connections are handled via onError callback
        if (hasMutex && this.recoveryManager) {
            this.recoveryManager.handleConnectionError(serverId, error);
        } else if (hasMutex && !this.recoveryManager) {
            // Fallback if no RecoveryManager during connection attempt
            if (ErrorClassifier.isAuthenticationError(errorMsg)) {
                this.setConnectionState(serverId, CONNECTION_STATES.AUTH_FAILED);
                this.log.error(`Authentication failed permanently for ${serverId} (no recovery manager)`);
            } else {
                this.setConnectionState(serverId, CONNECTION_STATES.NETWORK_ERROR);
            }
        }

        // Always notify via callback for error handling coordination
        if (this.onError) {
    ...
    async forceCleanupConnection(serverId) {
        ...
        if (this.clearRecoveryCallbacksForServer) {
            // Diagnostic: surface this as info so it's visible in normal logs when callbacks are removed
            this.log.info(`Clearing recovery callbacks before cleanup for ${serverId}`);
            this.clearRecoveryCallbacksForServer(serverId);
        }

        if (this.cancelRecoveryForServer) {
            this.log.info(`Canceling recovery attempts for ${serverId}`);
            this.cancelRecoveryForServer(serverId);
        }
