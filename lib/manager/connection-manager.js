...
class ConnectionManager {
    ...
    setConnectionState(serverId, state) {
        ...
        this.lastStateChange = { serverId, state, timestamp, oldState };
            ...
            // Avoid scheduling callbacks during shutdown/cleanup
            if (this.destroyed) {
                this.log.debug(`Skipping statusChangeCallback scheduling for ${serverId} due to destroyed state`);
                return;
            }

            // Capture callback to avoid referencing this.statusChangeCallback after cleanup nullifies it
            const cb = this.statusChangeCallback;
            if (typeof cb === 'function') {
                const nodeStatus = this.mapStateToNodeStatus(state);
                setTimeout(() => {
                    try {
                        // If destroyed in the meantime, skip silently
                        if (this.destroyed) return;
                        cb(serverId, nodeStatus);
                    } catch (err) {
                        // During shutdown the callback target may already be gone; keep it quiet
                        this.log.debug(`statusChangeCallback skipped: ${err.message}`);
                    }
                }, 0);
        }
    }
