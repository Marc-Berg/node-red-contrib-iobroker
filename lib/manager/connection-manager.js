...
class ConnectionManager {
    ...
    clearAllTimers() {
        // Clear all connection mutexes which may contain timeouts
    ...
    async forceCleanupConnection(serverId) {
        ...
        if (client) {
            try {
                ...
                if (client.pendingOperations && client.pendingOperations.size > 0) {
                    this.log.debug(`Waiting for ${client.pendingOperations.size} pending operations`);
                    await this.waitForPendingOperations(client, 3000);
                }

                client.destroy();
            } catch (error) {
                this.log.error(`Error destroying client for ${serverId}: ${error.message}`);
                this.forceClientDestroy(client);
            }
        }
        ...
        this.log.debug(`Cleanup completed for ${serverId}`);
    }
