...
class NodeRegistrationHelpers {
    ...
    /**
     * Setup delayed registration with orchestrator and event listeners
     */
    static setupDelayedRegistrationWithListeners(node, handlers, delay = 300) {
        setTimeout(() => {
            NodeRegistrationHelpers.registerWithOrchestrator(node);
            NodeRegistrationHelpers.registerEventListeners(node, handlers);
        }, delay);
    }
