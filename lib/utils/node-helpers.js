...
class NodeHelpers {
     ...
     */
    static shouldSendByAck(ack, filter) {
        switch (filter) {
    ...
    /**
     * Format value for status display with truncation
     */
    static formatValueForStatus(value, maxLength = 20) {
        let displayValue;
        ...
        if (displayValue.length > maxLength) {
            return "..." + displayValue.slice(-maxLength);
        }
    ...
    /**
     * Safe wrapper to get a message property supporting dot-notation.
     * Uses RED.util.getMessageProperty when available, otherwise a simple fallback.
     */
    static getMessageProperty(RED, msgObj, path) {
        if (!path || typeof path !== 'string') return undefined;
        if (RED && RED.util && typeof RED.util.getMessageProperty === 'function') {
            try {
                return RED.util.getMessageProperty(msgObj, path);
            } catch (e) {
                // fallback below
            }
        }
    ...
    /**
     * Safe wrapper to set a message property supporting dot-notation.
     * Uses RED.util.setMessageProperty when available, otherwise a simple fallback that creates nested objects.
     */
    static setMessageProperty(RED, msgObj, path, value) {
        if (!path || typeof path !== 'string') return;
        if (RED && RED.util && typeof RED.util.setMessageProperty === 'function') {
            try {
                RED.util.setMessageProperty(msgObj, path, value);
                return;
            } catch (e) {
                // fallback below
            }
