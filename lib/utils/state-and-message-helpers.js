...
class StateMessageHelpers {
    ...
    /**
     * Parse multiple states string into array
     */
    static parseMultipleStates(multipleStates) {
        if (!multipleStates || typeof multipleStates !== 'string') return [];
     ...
     * Validate state configuration
     */
    static validateStateConfiguration(inputMode, stateId, multipleStates) {
        const errors = [];
        ...
        if (inputMode === 'single') {
            ...
            if (!multipleStates || multipleStates.length === 0) {
                errors.push('Multiple states are required for multiple input mode');
            }
        }
        ...
        return { 
            ...
            errors: errors,
            isValid: errors.length === 0,  // Backward compatibility
            message: errors.length > 0 ? errors[0] : 'Configuration valid'  // Backward compatibility
        };
    }
