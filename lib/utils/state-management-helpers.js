    ...
    static validateStateConfiguration(inputMode, stateId, statesList) {
        ...
        if (inputMode === 'single') {
            ...
            if (!statesList || statesList.length === 0) {
                ...
                // Check for wildcards in multiple states list
                const wildcardStates = statesList.filter(state => state.includes('*'));
                if (wildcardStates.length > 0) {
                    errors.push(`Wildcard patterns not allowed in multiple states mode: ${wildcardStates.join(', ')}`);
                }
            }
        } else {
