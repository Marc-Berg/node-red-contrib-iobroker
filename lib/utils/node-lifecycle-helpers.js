...
class NodeLifecycleHelpers {
    ...
    static setupEventListeners(node, eventHandlers) {
        ...
        if (eventHandlers.onSubscriptionConfirmed) {
            Orchestrator.on('state:subscription_confirmed', eventHandlers.onSubscriptionConfirmed);
        }
        
        if (eventHandlers.onStateChanged) {
            Orchestrator.on('state:changed', eventHandlers.onStateChanged);
        }
        
        if (eventHandlers.onInitialStateValue) {
            Orchestrator.on(`state:initial_value:${node.id}`, eventHandlers.onInitialStateValue);
        }
    ...
    static setupCloseHandler(node, eventHandlers, cleanupCallbacks = []) {
        ...
        // Node-specific listeners
        if (eventHandlers.onSubscriptionConfirmed) {
            Orchestrator.removeListener('state:subscription_confirmed', eventHandlers.onSubscriptionConfirmed);
        }
        
        if (eventHandlers.onStateChanged) {
            Orchestrator.removeListener('state:changed', eventHandlers.onStateChanged);
        }
        
        if (eventHandlers.onInitialStateValue) {
            Orchestrator.removeListener(`state:initial_value:${node.id}`, eventHandlers.onInitialStateValue);
        }
