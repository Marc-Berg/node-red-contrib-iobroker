...
class ErrorAndLoggerHelpers {
     ...
     */
    static updateConnectionStatus(node, status, text = '') {
        switch (status) {
            case 'ready':
                node.status({ fill: "green", shape: "dot", text: text || "Ready" });
                break;
            case 'connecting':
            ...
            case 'disconnected':
                ...
                break;
            case 'retrying':
                node.status({ fill: "yellow", shape: "ring", text: text || "Retrying..." });
                break;
            case 'error':
                node.status({ fill: "red", shape: "ring", text: text || "Error" });
                break;
            case 'waiting':
                node.status({ fill: "grey", shape: "ring", text: text || "Waiting..." });
                break;
            case 'requesting':
                node.status({ fill: "blue", shape: "dot", text: text || "Requesting..." });
                break;
            case 'subscribing':
                node.status({ fill: "blue", shape: "ring", text: text || "Subscribing..." });
                break;
            default:
                node.status({ fill: "grey", shape: "ring", text: text || status });
        }
    }

    /**
     * Update status for multiple states progress
     */
    static updateInitialValuesProgress(node, receivedCount, totalCount) {
        ...
        node.status({ 
            fill: "yellow", 
            shape: "ring", 
            text: `Loading ${receivedCount}/${totalCount} (${percentage}%)` 
        });
    }

    /**
     * Update status for single state
     */
    static updateSingleStateStatus(node, stateId, value, filterMode, isInitial = false) {
        const filterText = filterMode === 'all' ? '' : ` (${filterMode})`;
        const initialText = isInitial ? ' (initial)' : '';
        const displayValue = value !== null && value !== undefined ? value : 'null';
        
        node.status({ 
            fill: "green", 
            shape: "dot", 
            text: `${displayValue}${initialText}${filterText}` 
        });
    }
    ...
    static updateMultipleStatesStatus(node, statesList, outputMode, filterMode) {
        const count = statesList.length;
        const modeText = outputMode === 'grouped' ? 'grouped' : 'individual';
        const filterText = filterMode === 'all' ? '' : ` (${filterMode})`;
        node.status({ 
            fill: "green", 
            shape: "dot", 
            text: `${count} states ${modeText}${filterText}` 
        });
    }
}
