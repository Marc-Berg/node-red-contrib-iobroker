    ...
    static createOperationExecutor(connectionManager, log) {
        return {
            executeOperation: async (serverId, operation, operationName = 'operation') => {
                try {
                    const isImportant = this.isImportantOperation(operationName);
                    
                    if (isImportant) {
                        log.info(`Executing ${operationName} for ${serverId}`);
                    } else {
                    ...
                    if (isImportant) {
                        log.info(`${operationName} completed for ${serverId}`);
                    } else {
            ...
            isImportantOperation: (operationName) => {
                const importantOps = [
                    'setState', 'getState', 'getStates', 'getObjects', 'subscribe', 
                    'getHistory', 'sendToAdapter', 'setObject', 'subscribeObjects',
                    'subscribeToLogs'
                ];
