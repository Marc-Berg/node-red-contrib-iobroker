...
class WebSocketManager {
    ...
    log(msg) {
        console.log(`[info] [WebSocket Manager] ${msg}`);
    }

    error(msg) {
        console.error(`[error] [WebSocket Manager] ${msg}`);
    }
    ...
    async synchronizeStateValues(serverId, isInitialConnection = false) {
        try {
            ...
            this.log(`Synchronizing ${statesToSync.size} states for ${serverId}`);
    ...
    scheduleReconnect(serverId) {
        ...
        // NEW: More lenient delay calculation for server unavailability
        let delay = Math.min(5000 * Math.pow(1.5, Math.min(recentFailures - 1, 5)), 60000);
        
        // Special handling for likely server unavailability
        if (connectionInfo.lastAuthError) {
            const errorMsg = connectionInfo.lastAuthError.toLowerCase();
            const isServerUnavailable = errorMsg.includes('timeout') || 
                                       errorMsg.includes('refused') || 
                                       errorMsg.includes('enotfound') || 
                                       errorMsg.includes('ehostunreach');
            
            if (isServerUnavailable) {
                // For server unavailability, use shorter delays and don't give up easily
                delay = Math.min(3000 * Math.pow(1.3, Math.min(recentFailures - 1, 8)), 30000);
            }
        }
    ...
    shouldRetryConnection(serverId, error) {
        ...
        if (connectionInfo) {
            ...
            // NEW: More lenient retry limit for server unavailability
            const errorMsg = error.message || error.toString();
            const isServerUnavailable = errorMsg.toLowerCase().includes('timeout') || 
                                       errorMsg.toLowerCase().includes('refused') || 
                                       errorMsg.toLowerCase().includes('enotfound') || 
                                       errorMsg.toLowerCase().includes('ehostunreach');
            
            const maxRetries = isServerUnavailable ? 20 : 10;
    ...
    async subscribe(nodeId, serverId, stateIdOrPattern, callback, config) {
        try {
            ...
            const connectionInfo = this.connections.get(serverId);
            if (connectionInfo && connectionInfo.connected && !stateIdOrPattern.startsWith('_dummy_')) {
    ...
    async unsubscribe(nodeId, serverId, stateIdOrPattern) {
        try {
            ...
            if (nodeIds.size === 0) {
                ...
                const connectionInfo = this.connections.get(serverId);
                if (connectionInfo && connectionInfo.client && connectionInfo.connected && !stateIdOrPattern.startsWith('_dummy_')) {
