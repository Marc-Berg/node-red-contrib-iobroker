...
class WebSocketManager {
    ...
    async createConnection(serverId, config) {
        ...
        try {
            ...
            client.on('connect', () => {
                ...
                connectionInfo.authenticationInfo.authenticated = useAuth;
                connectionInfo.connectionStats.totalConnects++;
                
                // Update token validity for OAuth2 connections
                if (useAuth && client.accessToken) {
                    connectionInfo.authenticationInfo.tokenObtainedAt = Date.now();
                    // OAuth2 tokens typically expire after 1 hour, but we don't get exact expiry
                    connectionInfo.authenticationInfo.tokenValidUntil = Date.now() + (60 * 60 * 1000);
                }
            ...
            client.on('reconnect', () => {
                ...
                // Update token info on reconnect
                if (useAuth && client.accessToken) {
                    connectionInfo.authenticationInfo.tokenObtainedAt = Date.now();
                    connectionInfo.authenticationInfo.tokenValidUntil = Date.now() + (60 * 60 * 1000);
                }
            ...
            client.on('error', (err) => {
                ...
                connectionInfo.connected = false;
                connectionInfo.authenticationInfo.authenticated = false;
                connectionInfo.authenticationInfo.lastAuthError = err;
                
                // Track authentication-specific errors
                if (err.includes('authentication') || err.includes('unauthorized') || err.includes('forbidden')) {
            ...
            connectionInfo.connecting = false;
            connectionInfo.connected = false;
            connectionInfo.authenticationInfo.lastAuthError = error.message;
    ...
    getConnectionStatus(serverId) {
        ...
        if (!connectionInfo) {
            return {
                ...
                authentication: {
                    ...
                    username: null,
                    tokenValid: false,
                    lastError: 'No connection info available'
                },
                connectionInfo: {
                    created: null,
                    lastConnect: null,
        ...
        const now = Date.now();
        const tokenValid = connectionInfo.authenticationInfo.tokenValidUntil 
            ? now < connectionInfo.authenticationInfo.tokenValidUntil 
            : connectionInfo.authenticationInfo.method === 'none';
        ...
        return {
            ...
            authentication: {
                ...
                tokenObtainedAt: connectionInfo.authenticationInfo.tokenObtainedAt,
                tokenValidUntil: connectionInfo.authenticationInfo.tokenValidUntil,
                lastError: connectionInfo.authenticationInfo.lastAuthError
            },
            connectionInfo: {
            ...
            serverDetails: {
                host: connectionInfo.config.iobhost,
                port: connectionInfo.config.iobport,
                useAuth: connectionInfo.authenticationInfo.method !== 'none'
            }
        };
    }
