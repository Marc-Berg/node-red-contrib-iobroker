// Enhanced websocket-manager.js with object management capabilities
...
class SimpleWebSocketManager {
    ...
    async subscribeObjects(client, pattern) {
        return new Promise((resolve, reject) => {
            const timeout = setTimeout(() => reject(new Error('Subscribe objects timeout')), 5000);
    ...
    async getObjects(serverId, pattern = '*') {
        const client = await this.getConnection(serverId, this.config);
        return new Promise((resolve, reject) => {
            const timeout = setTimeout(() => reject(new Error('Get objects timeout')), 15000);
