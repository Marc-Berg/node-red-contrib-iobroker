// websocket-manager.js - Simple singleton WebSocket manager
...
class SimpleWebSocketManager {
    ...
    async getState(serverId, stateId) {
        ...
        return new Promise((resolve, reject) => {
            const timeout = setTimeout(() => reject(new Error('Get timeout')), 10000);
    ...
    async setState(serverId, stateId, value, ack = true) {
        const client = await this.getConnection(serverId, this.config);
        return new Promise((resolve, reject) => {
            const timeout = setTimeout(() => reject(new Error('Set timeout')), 8000);
    ...
    async resetConnection(serverId, config) {
        this.log(`Resetting connection for ${serverId}`);
        this.close();
        await this.getConnection(serverId, config);
    }
