...
class WebSocketManager {
    ...
    async createConnection(serverId, config) {
        ...
        try {
            ...
            const connectionInfo = {
                ...
                status: 'connecting' // Track connection status
            };
    ...
    handleConnectionError(serverId, error) {
        ...
        if (connectionInfo) {
            connectionInfo.status = 'failed';
            connectionInfo.failedAt = Date.now();
            connectionInfo.lastError = error.message || error.toString();
        }
    ...
    // Enhanced method to send initial value for specific node with better connection checking
    async sendInitialValueForNode(serverId, stateId, nodeId, isDeployConnection = false) {
        try {
            const connectionInfo = this.connections.get(serverId);
            if (!connectionInfo) {
                throw new Error(`No connection info for ${serverId}`);
            }
            ...
            if (!connectionInfo.client.connected) {
                throw new Error(`Client not connected for ${serverId} (status: ${connectionInfo.status || 'unknown'})`);
            }
            
            // Double-check connection status from client statistics
            const clientStats = connectionInfo.client.getConnectionStats ? connectionInfo.client.getConnectionStats() : {};
            if (!clientStats.connected) {
                throw new Error(`Connection not ready according to client stats for ${serverId}`);
            }
            ...
            throw error; // Re-throw to allow retry logic in calling code
        }
    }
    ...
    getConnectionStatus(serverId) {
        ...
        if (connectionInfo.status === 'connecting') {
            ...
            status = 'connecting';
        } else if (client && client.connected && connectionInfo.status === 'connected') {
            status = 'connected';
            connected = true;
        } else if (client) {
        ...
        return {
            ...
            connectionInfo: {
                ...
                connected: connectionInfo.connectedAt || null,
