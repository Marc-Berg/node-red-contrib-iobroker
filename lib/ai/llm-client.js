...
/**
 * LLMClient – thin abstraction over OpenAI-compatible APIs (OpenAI, Azure, Ollama, generic compat).
 *
 ...
 *
 * Only Node built-ins are used; no additional npm dependency is required.
 */
 ...
 */
function jsonPost(url, body, headers, timeoutMs = 30000, rejectUnauthorized = true) {
...
class LLMClient {
    /**
     * @param {object} opts
     * @param {'openai'|'ollama'|'azure'|'openai_compatible'} opts.provider
     * @param {string} opts.model
     * @param {string} [opts.apiKey]       – required for openai / azure / openai_compatible
     * @param {string} [opts.baseUrl]      – required for ollama / azure / openai_compatible
     ...
     * @param {boolean} [opts.allowInsecureTls] – allow self-signed certs for HTTPS endpoints
     */
    constructor(opts = {}) {
        ...
        this.allowInsecureTls = opts.allowInsecureTls === true;
    }
    ...
    _shouldRejectUnauthorized(url) {
        const parsed = new URL(url);
        if (parsed.protocol !== 'https:') return true;

        // Explicit opt-in to insecure TLS.
        if (this.allowInsecureTls) return false;

        // Safe default: keep verification enabled everywhere.
        // Convenience exception: local Ollama over HTTPS often uses self-signed certs.
        if (this.provider === 'ollama' && this._isLocalHost(parsed.hostname)) {
            return false;
        }
    ...
    isConfigured() {
        ...
        if (this.provider === 'openai_compatible' && (!this.apiKey || !this.baseUrl)) return false;
        return true;
    }
    ...
    _endpoint() {
        switch (this.provider) {
            ...
            case 'openai_compatible':
                ...
                // If full endpoint is provided, keep it as-is.
    ...
    async chat(messages) {
        ...
        const rejectUnauthorized = this._shouldRejectUnauthorized(url);

        const data = await jsonPost(url, body, headers, this.timeoutMs, rejectUnauthorized);
